...
class SBNGraph(BaseGraph):
    ...
    def to_matrix(self)-> SBNGraph:  # raw/active/passive
...
if __name__ == '__main__':
    train_file = sys.argv[1] ## ocessed train data
    ### get all the information (nodes and edges) in the training data
    filedata = open(train_file, 'r')
    dataset = '.'.join(train_file.split('.')[:-2])
    with open(dataset + '-src-nodes.txt', 'w+', encoding='utf8') as nodes, \
            open(dataset + '-src-node1.txt', 'w+', encoding='utf8') as node1, \
            open(dataset + '-src-node2.txt', 'w+', encoding='utf8') as node2:
        for sbn in filedata:
            ### get sequence sbn data from preprocessed sbn data
            sbn_graph = SBNGraph().from_string(sbn.strip(), is_single_line=True)
            node_dict, edges = sbn_graph.to_matrix()
            ### get all the tuples of sbn
            traverse, graph_matrix = get_matrix(node_dict, edges)
            sbn_graph.to_png("sbn2graph1.png")
            ###
            nodes_sbn = traverse
            edges_sbn = graph_matrix
            print(edges_sbn)
            print(nodes_sbn)
            ###
            source_nodes_out = []
            ...
            # 1.
            nodes_sbn = [x.strip('^').strip('"') for x in nodes_sbn]
            source_nodes_out.extend(nodes_sbn)
            # 2. 邻接矩阵转化为深度遍历矩阵
            matrix = np.zeros((len(nodes_sbn),len(nodes_sbn)))
            for (i, j) in edges_sbn:
                matrix[i, j] = 1
            ## 3. 得到新的edge index
            new_edges_sbn = np.argwhere(matrix==1)
            ### 4. 得到新的数据
            for x in new_edges_sbn:
                source_edges_out_node1.append(str(x[0]))
                source_edges_out_node2.append(str(x[1]))
            nodes.write(' '.join(source_nodes_out) + '\n')
            node1.write(' '.join(source_edges_out_node1) + '\n')
            node2.write(' '.join(source_edges_out_node2) + '\n') 
