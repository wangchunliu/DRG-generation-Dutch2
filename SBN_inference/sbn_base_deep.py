...
class SBNData:
    def __init__(self, datatype, traverse, graph_reent):
        self.idx = 0
        ...
        if "deep" in self.data_type:
            ...
            x = self.new_matrix[0]
            y = x.T
            z = torch.IntTensor(len(self.traverse), len(self.traverse)).zero_()
            z[:, :] = torch.eye(len(self.traverse))
            listM.append(x)
            listM.append(y)
            listM.append(z)
...
if __name__ == '__main__':
    sbn = 'person.n.01 EQU speaker know.v.01 Proposition >1 Experiencer -1 Time +1 time.n.08 EQU now ATTRIBUTION <1 location.n.01 EQU ? male.n.02 be.v.03 Location -2 Theme -1 Time +1 time.n.08 EQU now'
    sbn_graph = SBNGraph().from_string(sbn, is_single_line=True)
    sbn_graph.to_png("sbn2graph1.png")
    matrix = sbn_graph.to_matrix()

    sbn = 'person.n.01 EQU speaker know.v.01 Proposition >1 Experiencer -1 Time +1 time.n.08 EQU now ATTRIBUTION <1 male.n.02 location.n.01 EQU ? be.v.03 Location -1 Theme -2 Time +1 time.n.08 EQU now'
    sbn_graph = SBNGraph().from_string(sbn, is_single_line=True)
    sbn_graph.to_png("sbn2graph2.png")


# ----------------------------To test in this file--------------------------------

def make_SBN_iterator_from_file(data_type, path):
    with codecs.open(path, "r", "utf-8") as corpus_file:
        for line in corpus_file:
            sbn = line.strip().split()
            sbn_graph = SBNGraph().from_string(sbn, is_single_line=True)
            traverse, graph_reent = sbn_graph.to_matrix()
            return SBNData(data_type, traverse, graph_reent)

## data_type: localSBNraw, localSBNbos, localSBNedge, localSBNself
## data_type: deepSBNraw, deepSBNbos, deepSBNedge, deepSBNself
make_SBN_iterator_from_file('localSBNbosReverse', 'sbn.txt')
