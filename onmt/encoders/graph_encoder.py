...
from torch_geometric.nn import GATConv, GINConv, GatedGraphConv, GCNConv
...
class GraphEncoder(EncoderBase):
    ...
    def __init__(self, embeddings, word_vec_size, gnn_type, gnn_layers, rnn_size):
        ...
        if self.gnn_type == 'ggnn':
            ...
            self.gnn_td = GCNConv(self.num_inputs, self.gnn_layers)
            self.gnn_bu = GCNConv(self.num_inputs, self.gnn_layers)
    ...
    def forward(self, src, lengths, batch):
        ...
        if self.gnn_type != 'gin':
            ...
            #print(batch_geometric.edge_index.shape)
        ...
        print(h__2.shape)
        print(memoery_bank)
        return (h__1, h__2), memory_bank1, lengths
        #######
        # ###### new format
        # memory_bank = unpack(memory_bank)[0]
        # h, _ = torch.max(memory_bank, 0)
        # # h = h.unsqueeze(0)
        # encoder_final = (h, h)
